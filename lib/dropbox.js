...
// Extraer precio del nombre de la carpeta/categoría
// Ejemplos: "Argollas de 15 mil" → 15000, "Conjuntos de 21 mil" → 21000
// "Anillos de 150mil" → 150000, "Collares 8 mil" → 8000
function extractPriceFromCategory(categoryName) {
  const match = categoryName.match(/(\d+)\s*mil/i);
  if (match) {
    return parseInt(match[1]) * 1000;
  }
  // Intentar número sin "mil" (precio directo)
  const directMatch = categoryName.match(/(\d+)/);
  if (directMatch && parseInt(directMatch[1]) > 100) {
...
export async function getCatalog() {
  try {
    ...
    const catalogPromises = categories.map(async (cat) => {
      ...
      const categoryPrice = extractPriceFromCategory(cat.name);
      ...
      const productsWithImages = await Promise.all(
        products.map(async (product) => {
          ...
          // Prioridad: precios.json > precio de categoría
          ...
          return {
            ...
            category: cat.name,
            img: imageUrl,
            price: priceInfo.price || categoryPrice || null,
            desc: priceInfo.desc || "",

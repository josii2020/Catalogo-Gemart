...
async function buildCatalog() {
  ...
  // Buscar imagen de banner en la raíz
  const bannerFile = entries.find((e) => {
    if (e[".tag"] !== "file") return false;
    const name = e.name.toLowerCase();
    return name.startsWith("banner") && imageExts.includes(name.slice(name.lastIndexOf(".")));
  });

  let bannerImg = null;
  if (bannerFile) {
    try { bannerImg = await getTemporaryLink(bannerFile.path_lower); } catch { bannerImg = null; }
  }
